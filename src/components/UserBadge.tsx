...
import { Shield, ShieldCheck, Crown, UserCheck, BadgeCheck } from "lucide-react";
...
const levelConfig: Record<UserLevel, { label: string; icon: typeof Shield; className: string }> = {
  ...
  2: { label: "통장", icon: ShieldCheck, className: "bg-badge-manager/10 text-badge-manager border-badge-manager/20" },
...
export function UserBadge({ level, className, isVerified }: UserBadgeProps) {
  ...
  return (
    <span className={cn("inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs font-medium", config.className, className)}>
      ...
      {config.label}
      {isVerified && <BadgeCheck className="h-3 w-3 text-primary ml-0.5" />}
    </span>
  );
}
...
export function VerifiedBadge() {
  return (
    <span className="inline-flex items-center gap-1 rounded-full bg-primary/10 text-primary border border-primary/20 px-2 py-0.5 text-xs font-medium">
      <BadgeCheck className="h-3 w-3" /> 실거주자
    </span>
  );
}
